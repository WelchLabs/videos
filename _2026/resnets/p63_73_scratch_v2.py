...
class ViTStream(InteractiveScene):
    ...
    depth_pool=16                      #1536 -> 384 channels
    ...
    #Voxel selection
    act_pct=90                        #keep the top (100-pct)% of cells per channel. 40x384x37x37 cells:
    act_alpha=0.80                   # pct=99 -> ~210k voxels, pct=97 -> ~630k. Start high.
    ...
    def collapse_to_grid(self):
        ...
        extra=[FadeOut(self.block_graphics)] if getattr(self, 'block_graphics', None) is not None else []
        self.play(self.frame.animate.reorient(*self.front_view()),
                  LaggedStart(*glides, lag_ratio=self.lag_ratio), *extra, run_time=self.move_time)
