...
class SpinShift(Animation):
    """SpinTo + a drift applied over the same beat; pivot travels with the drift."""
    def __init__(self, mobject, angle, axis, about_point, shift, **kwargs):
        self.angle=angle; self.axis=axis
        self.p0=np.asarray(about_point, dtype=np.float64)
        self.dv=np.asarray(shift, dtype=np.float64)
        self.prev=0.0
        super().__init__(mobject, **kwargs)
    def create_starting_mobject(self):
        return self.mobject
    def interpolate_mobject(self, alpha):
        a=self.rate_func(alpha)
        if a==self.prev: return
        self.mobject.rotate((a-self.prev)*self.angle, axis=self.axis,
                            about_point=self.p0+self.prev*self.dv)
        self.mobject.shift((a-self.prev)*self.dv)
        self.prev=a
...
class ViTStream(InteractiveScene):
    ...
    depth_pool=32                      #1536 -> 384 channels
    ...
    #Voxel selection
    act_pct=99.9            # Put at lik 90 before final render         #keep the top (100-pct)% of cells per channel. 40x384x37x37 cells:
    ...
    def construct(self):
        ...
        #Eh?
        # self.img.shift([-5, 0, 0])
        # self.image_border.shift([-5, 0, 0])
        ...
        # Stage A: spin in place (transformer-block graphics fade out here)
        # img_group=Group(self.img, self.image_border)

        spins=[]
        # ic=np.array([0.5*self.image_depth, 0.0, 0.0])
        # spins.append(AnimationGroup(SpinTo(self.img, -90*DEGREES, OUT, ic),
        #                             # self.img.animate.shift([-5, 0, 0]),
        #                             SpinTo(self.image_border, -90*DEGREES, OUT, ic)))
        #                             # self.image_border.animate.shift([-5, 0, 0])))
        img_shift=np.array([-11.0, 0, 0])          # tune me
        ic=np.array([0.5*self.image_depth, 0.0, 0.0])
        ic_final=ic+img_shift
        spins.append(AnimationGroup(SpinShift(self.img, -90*DEGREES, OUT, ic, img_shift),
                                    SpinShift(self.image_border, -90*DEGREES, OUT, ic, img_shift)))
        ...
        self.play(LaggedStart(*spins, lag_ratio=self.lag_ratio), *self.fade_graphics(),
                  run_time=2.0) #Increase for final animation
        ...
        self.wait()
        self.play(LaggedStart(*squashes, lag_ratio=self.lag_ratio), run_time=2.0)
        ...
        img_outline=None
        self.remove(self.image_border)
        img_outline=front_square(ic_final[0], ic_final[2], self.image_w, self.grid_y, CHILL_BROWN, line_radius)
        ...
        # self.play(self.frame.animate.reorient(0, 90, 0, (np.float32(447.19), np.float32(325.86), np.float32(12.04)), 830.52),
        #          run_time=2.0) #Increase
        ...
        self.wait(1.0)
        self.play(self.frame.animate.reorient(*self.front_view()),
                  LaggedStart(*moves, lag_ratio=self.lag_ratio), run_time=2.0)
