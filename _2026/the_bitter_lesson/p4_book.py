...
json_dir=Path('/Users/stephen/videos/_2026/the_bitter_lesson')
...
class P4_book_1(InteractiveScene):
    def construct(self):
        ...
        for label, idx, x, y in all_nodes_list:
            ...
            box.set_fill(opacity=1.0, color=WHITE) 
            box.set_stroke(width=0.5)
        ...
        for start_idx, end_idx in all_edges:
            ...
            arrow = Arrow(start_point, end_point, buff=0, thickness=6)
        ...
        self.frame.reorient(90, 0, 0, (np.float32(9.84), np.float32(9.11), np.float32(0.02)), 301.69)
...
class P4Dots(InteractiveScene):
    def construct(self):
        ...
        # --- Same preprocessing as P4a so spacing is identical ---
        all_nodes_list = copy.deepcopy(small_graph_nodes[:-1])  # Leave off ending node for now
        for n in all_nodes_list:
            n[1] = n[1] + 100000  # Avoid collisions
            n[2] = n[2] * SCALE_FACTOR - GLOBAL_SHIFT[0]
        ...
        for node_data in data["nodes"][:nodes_to_render]:
            if node_data['highlighted']: continue
            label = node_data["phoneme"]
            idx = node_data["id"]
            x = node_data["x"] * JSON_SCALE_FACTOR_X - 3 - GLOBAL_SHIFT[0]
            y = node_data["y"] * JSON_SCALE_FACTOR_Y - GLOBAL_SHIFT[1]
            all_nodes_list.append([label, idx, x, y])
        ...
        # --- Render each node as a Dot instead of a labeled rectangle ---
        dot_radius = 0.35
        node_mobjects = {}
        for label, idx, x, y in all_nodes_list:
            dot = Dot(radius=dot_radius, color=CHILL_BROWN)
            dot.set_fill(CHILL_BROWN, opacity=1.0)
            dot.move_to([x, y, 0])
            node_mobjects[idx] = dot

        # Reindex edges to avoid collisions
        all_edges = list(np.array(small_graph_edges) + 100000)
        for edge_data in data["edges"]:
            if edge_data["source"] == 0:
                edge_data["source"] = 100000
        ...
        for start_idx, end_idx in all_edges:
            ...
            # Connect to the edge of each dot (with a small gap)
            gap = 0.05
            start_point = start_node.get_center() + (dot_radius + gap) * direction
            end_point = end_node.get_center() - (dot_radius + gap) * direction
        ...
        self.remove(non_priority_arrows_group)
        self.play(FadeOut(non_priority_nodes_group), run_time=7)
        self.wait()
