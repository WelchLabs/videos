# Original numpy manual gradients — RMSProp + discounting, no PyTorch autograd
...
def save_plot():
    ...
    ax.plot(log_episodes, log_rewards, alpha=0.3, color='mediumpurple', label='reward')
    ax.plot(log_episodes, log_running, color='mediumpurple', linewidth=2, label='running reward')
    ax.set_xlabel('episode')
    ax.set_ylabel('reward')
    ax.set_title('v4 — numpy manual grads + RMSProp + discount')
    ax.legend()
    fig.tight_layout()
...
print(f"H={H}, lr={learning_rate}, batch={batch_size}, numpy+RMSProp+discount")
...
try:
    while True:
        ...
        if done:
            ...
            epdlogp = np.asarray(dlogps, dtype=np.float32)
            ...
            epdlogp *= discounted_epr[:, np.newaxis]
