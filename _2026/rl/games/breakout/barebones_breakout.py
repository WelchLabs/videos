...
meta = {
    ...
    "description": "vanilla REINFORCE, Adam, whole-episode return, "
                   "running-reward baseline, no discount; auto-fire on life loss",
    "env_id": ENV_ID,
    "actions": ACTIONS,
...
if have_ckpt:
    ...
    best_reward = ckpt.get("best_reward", 0.0)
...
def save_plot():
    ...
    ax.set_title("episode reward (bricks broken)"); ax.set_xlabel("episode"); ax.legend()
    ax = axes[0, 1]
    ...
    fig.suptitle("Breakout - barebones REINFORCE (PyTorch, Adam, no discount)")
    fig.tight_layout()
...
def render_progress():
    ...
    obs, info = _progress_env.reset()
    obs, _, _, _, info = _progress_env.step(FIRE)
    lives = info["lives"]
    prev = None
    ...
    while True:
        ...
        obs, _, term, trunc, info = _progress_env.step(ACTIONS[a])
        steps += 1
        if steps % 3 == 0:
            frames.append(_progress_env.render())
        done = term or trunc
        if not done and info["lives"] < lives:
            obs, _, _, _, info = _progress_env.step(FIRE)
            prev = None
        lives = info["lives"]
...
def fire_reset(seed=None):
    obs, info = env.reset(seed=seed)
    obs, _, _, _, info = env.step(FIRE)
    return obs, info


observation, info = fire_reset(seed=SEED)
...
try:
    while True:
        ...
        observation, reward, terminated, truncated, info = env.step(ACTIONS[action_idx.item()])
        reward_sum += reward
        ep_steps += 1
        total_steps += 1
